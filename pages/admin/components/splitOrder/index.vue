		...
		<checkbox-group @change="checkboxChange">
			<block v-for="(item,index) in splitGoods" :key="index">
				<view class='items acea-row row-between-wrapper' v-if="item.surplus_num>0">
					<!-- #ifndef MP -->
					<checkbox  :value="(item.id).toString()" :checked="item.checked" :disabled="(type!=1&&item.surplus_num<1)?true:false" />
					<!-- #endif -->
					<!-- #ifdef MP -->
					<checkbox  :value="item.id" :checked="item.checked" :disabled="(type!=1&&item.surplus_num<1)?true:false" />
					<!-- #endif -->
					<view class='picTxt acea-row row-between-wrapper'>
						...
						<view class='carnum acea-row row-center-wrapper'>
							<view class="reduce iconfont icon-shangpinshuliang-jian" :class="item.surplus_num == 1 ? 'on' : ''" @click.stop='subCart(item)'></view>
							<view class='num'>{{item.surplus_num}}</view>
							<view class="plus iconfont icon-shangpinshuliang-jia" :class="item.surplus_num == item.numShow ? 'on' : ''" @click.stop='addCart(item)'></view>
						</view>
					</view>
				</view>
			</block>
