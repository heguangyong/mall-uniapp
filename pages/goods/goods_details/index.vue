		...
		<view class="product-con tui-skeleton" :style="{ visibility: showSkeleton ? 'hidden' : 'visible' }">
			<view class="navbar" :class="opacity > 0.6 ? 'bgwhite' : ''">
				<view class="navbarH" :style="'height:' + navH + 'rpx;'">
					<view class="navbarCon acea-row" :style="{ paddingRight: navbarRight + 'px' }">
						<nav-bar iconColor='#fff' :isBackgroundColor="false" ref="navBarRef" :isHeight="false">
						</nav-bar>
						<!-- 头部tab标题 -->
						<!-- #ifdef H5  || APP-PLUS-->
                  ...
                  " v-for="(item, index) in navList" :key="index" @tap="tap(index)">
						...
						<!-- #endif -->
						<!-- 分享 -->
						<view class="iconfont icon-ic_share1" :style="[shareStyle]"
			...
			<view class="footer acea-row row-between-wrapper">
                ...
                ">
                    ...
                    productPrice.redeemIntegral > integral
                      ? 'bg-color-hui'
                      : 'bg-red'
          ...
          ">
			...
			<productWindow :attr="attr" :isShow="1" :iSplus="1" @myevent="onMyEvent" @ChangeAttr="ChangeAttr"
				...
				:groupBuyActivityResponse="groupBuyActivityResponse" @buyLimit="buyLimit">
			</productWindow>
					...
					<view class="list">
						<view class="item acea-row" v-for="(item, index) in guaranteeList" :key="index">
							<view class="pictrue">
								...
								<text
									:class="(marketingType === ProductMarketingTypeEnum.Groupbuying||productType === ProductTypeEnum.Integral)?'activityIcon':'iconColor'"
									class="iconfont icon-gou2"></text>
							</view>
							<view class="text">
        ...
        this.$refs.navBarRef.currentPage = false;
			...
			checkedAttr(productSelect) {
				...
				this.$set(
					...
					"groupPrice",
					productSelect.groupPrice
				);
				this.$set(
					this.attr.productSelect,
					"groupStock",
					productSelect.groupStock
				);
				this.$set(
					this.attr.productSelect,
					"isPaidMember",
					this.productInfo.isPaidMember
				);
				this.$set(
					this.attr.productSelect,
					"redeemIntegral",
					productSelect.redeemIntegral
				);
				this.$set(this.attr.productSelect, "cost", productSelect.cost);
			...
			noCheckedAttr() {
				...
				this.$set(this.attr.productSelect, "groupPrice", this.productInfo.groupPrice);
				this.$set(
					this.attr.productSelect,
					"isPaidMember",
					this.productInfo.isPaidMember
				...
				this.$set(
					this.attr.productSelect,
					"redeemIntegral",
					this.productInfo.redeemIntegral
				);
				this.$set(this.attr.productSelect, "cost", this.productInfo.cost);
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id, that.marketingType, 0, this.groupActivityId)
					.then((res) => {
						...
						that.productType = productInfo.type; //商品类型 0=普通商品,1-积分商品,2-虚拟商品,4=视频号,5-云盘商品,6-卡密商品
						...
						let navList =
							that.productType !== ProductTypeEnum.Integral ? ["商品", "评价", "详情"] : ["商品", "详情"];
			...
			DefaultSelect: function() {
				...
				if (this.marketingType === this.ProductMarketingTypeEnum.Groupbuying) {
					const nonZeroDefault = productValues.find(item => item.isDefault && item.stock !== 0 && item
						.groupStock !== 0);
					if (nonZeroDefault) {
						value = productValues.find(item => item.isDefault && item.stock !== 0 && item.groupStock !== 0)
							.sku.split(',');
					} else {
						const haveStock = productValues.find(item => item.stock !== 0 && item.groupStock !== 0)
						if (haveStock) {
							value = productValues.find(item => item.stock !== 0 && item.groupStock !== 0).sku.split(
								',');
						} else {
							value = [];
						}
					...
					const nonZeroDefault = productValues.find(item => item.isDefault && item.stock !== 0);
					if (nonZeroDefault) {
						value = productValues.find(item => item.isDefault && item.stock !== 0).sku.split(',');
					} else {
						const haveStock = productValues.find(item => item.stock !== 0)
						if (haveStock) {
							value = productValues.find(item => item.stock !== 0).sku.split(',');
						} else {
							value = [];
						}
