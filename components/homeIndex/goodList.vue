	...
	export default {
		...
		data() {
			return {
				...
				type: this.dataConfig.tabConfig.tabVal || 0, //商品类型 0指定商品，1指定分类，2指定品牌，3指定商户
		...
		methods: {
			...
			getProList() {
				if (!this.dataConfig.goodsList.list.length) return;
			...
			productslist() {
				...
				if (this.merId) {
					...
					switch (this.type) {
						case 1:
							this.params.merId = '';
							this.params.brandId = '';
							this.params.cid = this.classifyId.join(',');
							break;
						case 2:
							this.params.cid = '';
							this.params.merId = '';
						...
						case 3:
							this.params.cid = '';
							this.params.brandId = '';
							this.params.merId = this.homeMerId.join(',');
							break;
					}
				}
				if (this.goodsSort === 0) {
				...
				!this.merId ? productList(this.params).then(res => {
